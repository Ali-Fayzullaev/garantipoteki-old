                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                      <input
                        ...
                        className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-brandOrange focus:border-transparent"
                        required
                      />
                    </div>
                    <div>
...
function LeadForm({ onSuccess }: { onSuccess?: () => void }) {
  ...
  // Используем ту же логику, что и в модальном окне
  const handleServiceSelect = async (service_id: number, service: string) => {
    if (!dealId) return;
    setIsLoading(true);
    try {
      await updateDeal(
        { service_id: service_id, phone_number: formData.phone },
        dealId
      );
